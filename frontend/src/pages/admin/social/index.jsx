...
function Index() {
	...
	useEffect(() => {
		axios.get('/admin/social').then(({ data }) => {
			setData(data.data)
		})
	}, []);
		...
		swal({
			...
			.then((willDelete) => {
				if (willDelete) {
					...
					axios.delete(`/admin/social/${id}/delete`).then(() => {
			...
			<div className="flex items-center justify-between pb-6 ">
				...
				<div className="flex items-center justify-between">
					...
					<div className="ml-10 space-x-8 lg:ml-40">
						<Link to="/admin/social/create" className="px-4 py-2 font-semibold tracking-wide text-white bg-indigo-600 rounded-md cursor-pointer">Create</Link>
					</div>
				</div>
			</div>
			<div>
							...
							<tbody>
								{data.map((item, index) => (
									<tr key={index}>
										...
										<td className="w-4/12 px-5 py-5 text-sm text-center bg-white border-b border-gray-200">
											<p className="text-gray-900 whitespace-no-wrap">{item.description}</p>
										</td>


										<td className="w-3/12 px-5 py-5 text-sm text-center bg-white border-b border-gray-200">
										...
										<td className="flex items-center justify-between w-full px-5 py-5 text-sm text-center bg-white border-gray-200">
											...
											<div>
												<Link to={{ pathname: `/admin/social/${item._id}/edit` }} className="p-2 px-4 text-white bg-blue-500 rounded" ><i className="fas fa-pen"></i></Link>
											</div>
										</td>
									</tr>
								))}
